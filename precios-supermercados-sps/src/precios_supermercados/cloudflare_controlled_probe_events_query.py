"""Detalle de trazas de la sonda controlada mediante ``view: events``.

La vista ``traces`` se usa únicamente para descubrir IDs candidatos dentro de
la ventana temporal firmada. El detalle se consulta como eventos individuales
filtrados por servicio + traceId y después se pasa al reconciliador estricto
existente. Ninguna respuesta de Cloudflare concede autoridad por sí misma.
"""
...
TRACE_EVENTS_QUERY_ID = "precios-sps-controlled-probe-events-v3"
...
def build_trace_events_query(
    *,
    ...
    """Solicita todos los eventos del trace candidato sin atributos custom server-side."""
...
def normalize_events_response(payload: Mapping[str, object]) -> tuple[Mapping[str, object], ...]:
    """Valida el contenedor ``result.events`` sin reescribir los eventos."""

    result = _successful_result(payload)
    container = _mapping(result.get("events"), "probe_observability_events_container_invalid")
    raw_events = _sequence(container.get("events"), "probe_observability_events_invalid")
    ...
    if count is not None:
        if isinstance(count, bool) or not isinstance(count, (int, float)) or int(count) != count or count < 0:
            _fail("probe_observability_count_invalid")
        if int(count) > len(raw_events):
            _fail("probe_observability_events_truncated")

    events: list[Mapping[str, object]] = []
    for index, raw in enumerate(raw_events):
        event = _mapping(raw, f"probe_event_{index}_invalid")
        if event.get("dataset") != OBSERVABILITY_DATASET:
...
class ControlledProbeEventsQueryTransport(Protocol):
    def post_json(
        self,
        path: str,
        *,
...
class ControlledProbeEventsVerifierClient:
    """Re-verifica receipt + trace usando ``traces`` para discovery y ``events`` para detalle."""

    def __init__(self, account_id: str, transport: ControlledProbeEventsQueryTransport) -> None:
    ...
    def reconcile_artifact(
        ...
        *,
        ...
        start = _timestamp(receipt.get("physical_started_at_utc"), "probe_receipt_start_invalid")
        end = _timestamp(receipt.get("response_completed_at_utc"), "probe_receipt_end_invalid")
        if end < start:
        ...
        for trace_id in trace_ids:
            ...
            events = normalize_events_response(detail)
