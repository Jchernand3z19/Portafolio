...
const SCENARIOS: Array<{ id: Scenario; title: string; description: string }> = [
  { id: 'valid', title: 'Comprobante válido', description: 'BAC + B4 C18 + monto y referencia legibles.' },
  { id: 'missingHome', title: 'Falta vivienda', description: 'El sistema conserva contexto y pregunta bloque/casa.' },
  { id: 'duplicate', title: 'Reenvío duplicado', description: 'La misma referencia no vuelve a sumar el pago.' },
  { id: 'editedConflict', title: 'Conflicto sospechoso', description: 'Misma referencia con datos diferentes: revisión.' },
];
...
export function DemoWorkbench() {
  ...
  const systemMessage = scenario === 'duplicate'
    ? 'ℹ️ Este comprobante ya había sido recibido.\nNo se registró un segundo pago.'
    : scenario === 'editedConflict'
      ? 'Recibimos tu comprobante y quedó en revisión. No se registrará como pago verificado hasta confirmar la transacción.'
      : extraction.home
        ? `✅ Comprobante recibido\nBloque ${extraction.home.block} · Casa ${extraction.home.house}\nL${extraction.amount?.toFixed(2)}\n07/09/2026\nEstado: pendiente de verificación.`
        : `Recibimos tu comprobante por L${extraction.amount?.toFixed(2)}, pero falta identificar la vivienda.\nPor favor responde con tu bloque y casa.\nEjemplo: B4 C18`;
      ...
      <section className="panel">
        <div className="phone">
          ...
          <div className="chat">
            ...
            {scenario === 'missingHome' && <div className="bubble bubble--user">B4 C18</div>}
            {scenario === 'missingHome' && <div className="bubble bubble--system">✅ Comprobante registrado para Bloque 4, Casa 18. Estado: pendiente de verificación.</div>}
          </div>
        </div>
        <div className="pipeline">
          {['Webhook', 'Archivo', 'OCR', 'BAC parser', 'Vivienda', 'Duplicados', status].map((step) => <span className="is-done" key={step}>{step}</span>)}
        </div>
        <div className="extraction">
          ...
          <div className="field"><span>Vivienda</span><strong>{extraction.home ? `B${extraction.home.block} C${extraction.home.house}` : 'No identificada'}</strong></div>
