...
PRODUCT_KEYS = {
    "availability", "brand", "category", "current_price", "ean",
    "is_promotion", "item_id", "presentation", "product_id", "reference",
    "reported_regular_price", "source_key", "source_key_type", "source_name",
}
...
def build_store_request() -> dict[str, object]:
    return {"business_partner": BUSINESS_PARTNER, "cityCode": None, "pickPosition": DELIVERY_TYPE}
...
def build_catalog_request(skip: int, take: int = PAGE_SIZE) -> tuple[str, dict[str, object]]:
    ...
    return f"{CATALOG_ENDPOINT}?skip={skip}&take={take}", {
        ...
        "search": "",
        "filter": {
            "priceMin": None, "priceMax": None, "brand": None, "supplier": None,
            "bulletPoint": None, "agency": None,
        },
        "source": "WEB",
        "hidden": "0",
    }
...
def _image_url(product: dict[str, Any]) -> str | None:
    ...
    active = [i for i in images if isinstance(i, dict) and i.get("active") == "1"]
    candidates = active or [i for i in images if isinstance(i, dict)]
    if not candidates:
        return None
    image = sorted(candidates, key=lambda i: i.get("position", 0))[0]
...
def _pricing(product: dict[str, Any], availability: str) -> tuple[str | None, str | None, bool | None, dict[str, Any]]:
    ...
    if current is None or Decimal(current) == 0:
        ...
        return None, None, None, {
            ...
            "source_new_price": product.get("newPrice"), "source_price": product.get("price"),
            "source_old_price": product.get("oldPrice"), "source_list_price": product.get("listPrice"),
            "source_discount": product.get("discount"), "source_discount_two": product.get("discountTwo"),
        }
    ...
    regular = candidates[0][1] if candidates else None
    promotion = regular is not None
    if any(v is not None and v != 0 for v in (discount, discount_two)) and not promotion:
    ...
    return current, regular, promotion, {
        ...
        "source_old_price": product.get("oldPrice"), "source_list_price": product.get("listPrice"),
        "source_discount": product.get("discount"), "source_discount_two": product.get("discountTwo"),
        "regular_price_evidence": [label for label, _ in candidates],
    }
...
def parse_products(products: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    for product in products:
        ...
        brand, category, unit = product.get("brandName"), product.get("materialGroupName"), product.get("unitMeasureCode")
        ...
        rows.append({
            "product_id": code, "item_id": code, "source_key_type": "sku", "source_key": code,
            "source_name": name, "brand": brand.strip() if isinstance(brand, str) and brand.strip() else None,
            "reference": code, "ean": None,
            ...
            "is_promotion": promotion, "availability": availability,
        })
        details[code] = {
            **pricing,
            ...
            "source_stock": product.get("stock"), "source_brand_id": product.get("brandId"),
            ...
            "source_is_adult": product.get("is_adult"), "image_url": _image_url(product),
        }
    return rows, details
...
def parse_catalog_page(payload: dict[str, Any], *, expected_skip: int, take: int = PAGE_SIZE) -> dict[str, Any]:
    ...
    require(len(data) == min(take, total - expected_skip), "page_item_count_mismatch")
    rows, details = parse_products(data)
    return {"total_items": total, "total_pages": pages, "item_per_page": item_per_page,
            "current_page": current_page, "rows": rows, "source_details": details}
...
def reconcile_capture(directory: str | Path) -> dict[str, Any]:
    ...
    for record in sorted(records, key=lambda item: item.get("skip", -1)):
        ...
        offsets.add(skip)
        require(record.get("status") in {200, 201} and record.get("retries") == 0, "page_request_failed")
        url, body = build_catalog_request(skip, take)
        require(record.get("url") == url, "page_url_mismatch")
        require(record.get("request_body") == body, "page_body_mismatch")
        filename, digest = record.get("response_file"), record.get("response_sha256")
        ...
        totals.add(parsed["total_items"]); page_counts.add(parsed["total_pages"])
        for row in parsed["rows"]:
            require(row["source_key"] not in all_details, "cross_page_source_code_duplicate")
            all_rows.append(row); all_details[row["source_key"]] = parsed["source_details"][row["source_key"]]
        ...
        page_evidence.append({"skip": skip, "take": take, "http_status": record.get("status"),
                              "response_sha256": digest, "response_bytes": len(raw)})
    require(len(totals) == len(page_counts) == 1, "catalog_total_changed_during_capture")
    total, total_pages = next(iter(totals)), next(iter(page_counts))
    ...
    availability_counts = dict(sorted(Counter(r["availability"] for r in all_rows).items()))
    promotion_counts = {
        "promotion": sum(r["is_promotion"] is True for r in all_rows),
        "not_promotion": sum(r["is_promotion"] is False for r in all_rows),
        "unknown": sum(r["is_promotion"] is None for r in all_rows),
    }
    membership_sha256 = hashlib.sha256("\n".join(r["source_key"] for r in all_rows).encode()).hexdigest()
    return {
        "result": "success", "supermarket_id": SUPERMARKET_ID, "location_id": LOCATION_ID,
        "city": CITY, "currency": CURRENCY, "scope": SCOPE, "store_id": STORE_ID,
        "office_code": OFFICE_CODE, "location_one_code": LOCATION_ONE_CODE,
        "location_two_code": LOCATION_TWO_CODE, "location_verified_same_run": True,
        "catalog_complete": True, "validation_passed": True, "observed_at_utc": observed_at,
        "catalog_products_reported": total, "unique_products_extracted": total,
        "membership_count": total, "membership_sha256": membership_sha256,
        "skus_extracted": len(all_rows),
        "skus_with_price": sum(r["current_price"] is not None for r in all_rows),
        "availability_counts": availability_counts, "promotion_counts": promotion_counts,
        "products": all_rows, "source_details": all_details, "page_evidence": page_evidence,
