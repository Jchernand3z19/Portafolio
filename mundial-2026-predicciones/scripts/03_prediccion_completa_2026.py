...
def obtener_codigos_concretos_clasificados_ronda_32(calendario, mapa_clasificados):
    """
    Detecta equipos concretos de Ronda de 32 que sí clasificaron.

    Sirve para reservarlos y evitar que el fallback use un equipo que
    ya aparece correctamente en otro partido del calendario.
    """
    partidos_r32 = extraer_partidos_fase(calendario, "Ronda de 32")
    ...
    for _, row in partidos_r32.iterrows():
        for lado in ["a", "b"]:
            ...
            if codigo in mapa_clasificados["clasificados_por_codigo"]:
                reservados.add(codigo)

    return reservados


def obtener_mejor_clasificado_disponible(
    mapa_clasificados,
    codigos_excluidos=None,
):
    """
    Devuelve el mejor clasificado disponible que todavía no se haya usado.

    Se usa como fallback cuando el calendario trae un equipo concreto
    que no clasificó y no existe slot_a / slot_b para resolver el cruce.
    """
    if codigos_excluidos is None:
        codigos_excluidos = set()

    disponibles = mapa_clasificados["clasificados_disponibles"]

    for equipo in list(disponibles):
        codigo = texto_limpio(equipo["codigo"]).upper()
        ...
        equipo = equipo.copy()
        equipo["origen_resolucion"] = "fallback_clasificado_disponible"
...
def remover_clasificado_disponible(mapa_clasificados, codigo):
    """
    Remueve un clasificado de la lista de disponibles para evitar duplicados.
    """
    codigo = texto_limpio(codigo).upper()
    ...
    for equipo in list(disponibles):
        if texto_limpio(equipo["codigo"]).upper() == codigo:
            disponibles.remove(equipo)
            return
...
def resolver_slot(
    ...
    texto_slot_extra="",
    codigos_excluidos=None,
    permitir_fallback_ronda_32=False,
):
    """
    ...
    - Si no clasificó y no hay slot real, se usa fallback:
      mejor clasificado disponible que todavía no haya sido usado.
    """
    if codigos_excluidos is None:
        codigos_excluidos = set()
    ...
    if tipo_slot == "primero_grupo":
        ...
        if equipo and equipo["codigo"] not in codigos_excluidos:
            remover_clasificado_disponible(mapa_clasificados, equipo["codigo"])
            equipo = equipo.copy()
            equipo["origen_resolucion"] = "slot_primero_grupo"
            return equipo
    ...
    if tipo_slot == "segundo_grupo":
        ...
        if equipo and equipo["codigo"] not in codigos_excluidos:
            remover_clasificado_disponible(mapa_clasificados, equipo["codigo"])
            equipo = equipo.copy()
            equipo["origen_resolucion"] = "slot_segundo_grupo"
            return equipo

        return None

    if tipo_slot == "tercero_grupo":
        equipo = resolver_tercero(
        ...
        if equipo:
            remover_clasificado_disponible(mapa_clasificados, equipo["codigo"])
            equipo = equipo.copy()
            equipo["origen_resolucion"] = "slot_mejor_tercero"
    ...
    if equipo_concreto(equipo_slot, codigo_slot):
        if fase == "Ronda de 32":
            ...
            if equipo is not None and codigo_slot not in codigos_excluidos:
                remover_clasificado_disponible(mapa_clasificados, codigo_slot)

                if equipo["tipo_clasificacion"] == "Mejor tercero":
                    remover_tercero_disponible(mapa_clasificados, codigo_slot)

                equipo = equipo.copy()
                equipo["origen_resolucion"] = "equipo_concreto_clasificado"
                return equipo

            if permitir_fallback_ronda_32:
                return obtener_mejor_clasificado_disponible(
                    mapa_clasificados=mapa_clasificados,
                    codigos_excluidos=codigos_excluidos,
                )
        ...
        return {
            ...
            "origen_resolucion": "equipo_concreto_calendario",
        }

    if fase == "Ronda de 32" and permitir_fallback_ronda_32:
        return obtener_mejor_clasificado_disponible(
            mapa_clasificados=mapa_clasificados,
            codigos_excluidos=codigos_excluidos,
        )
...
def simular_fase_eliminatoria_desde_calendario(
    ...
    - Si el calendario no tiene slot real y trae un no clasificado,
      se reemplaza por el mejor clasificado disponible como fallback.
    """
    ...
    codigos_reservados_r32 = set()

    if fase == "Ronda de 32":
        codigos_reservados_r32 = obtener_codigos_concretos_clasificados_ronda_32(
    ...
    for _, row in partidos_fase.iterrows():
        ...
        codigo_a_cal = texto_limpio(row["codigo_a"]).upper()
        codigo_b_cal = texto_limpio(row["codigo_b"]).upper()

        excluidos_a = set(codigos_usados_ronda_32)

        if fase == "Ronda de 32":
            for cod in codigos_reservados_r32:
                if cod != codigo_a_cal:
                    excluidos_a.add(cod)
        ...
        equipo_a_info = resolver_slot(
            ...
            codigos_excluidos=excluidos_a,
            permitir_fallback_ronda_32=(fase == "Ronda de 32"),
        )

        excluidos_b = set(codigos_usados_ronda_32)
        ...
        if fase == "Ronda de 32" and equipo_a_info is not None:
            excluidos_b.add(texto_limpio(equipo_a_info["codigo"]).upper())
        ...
        origen_a = texto_limpio(equipo_a_info.get("origen_resolucion", ""))
        origen_b = texto_limpio(equipo_b_info.get("origen_resolucion", ""))

        uso_fallback = (
            origen_a == "fallback_clasificado_disponible"
            or origen_b == "fallback_clasificado_disponible"
        )
        ...
        if uso_fallback:
            notas = (
                "Cruce resuelto con fallback porque el calendario no trae slot real "
                "y contenía un equipo no clasificado. "
...
def validar_ronda_32_contra_tabla_grupos(prediccion_completa_df, tabla_grupos_df):
    """
    Valida que la Ronda de 32 use exactamente los 32 clasificados simulados.

    Si encuentra:
    - equipos no clasificados
    - equipos duplicados
    - clasificados faltantes
    - cantidad incorrecta de partidos
    entonces detiene el script con error.
    """
    r32 = prediccion_completa_df[
        prediccion_completa_df["fase"] == "Ronda de 32"
    ].copy()

    if len(r32) != 16:
        raise ValueError(
            "Validación Ronda de 32 fallida: "
            f"se esperaban 16 partidos y se encontraron {len(r32)}."
        )

    clasificados = tabla_grupos_df[
        tabla_grupos_df["clasifica"] == "Sí"
    ].copy()

    no_clasificados = tabla_grupos_df[
        tabla_grupos_df["clasifica"] != "Sí"
    ].copy()

    codigos_clasificados = set(
        clasificados["codigo"].astype(str).str.strip().str.upper()
    )

    codigos_no_clasificados = set(
        no_clasificados["codigo"].astype(str).str.strip().str.upper()
    )
        ...
        participantes.append(
            {
                ...
                "lado": "A",
                "seleccion": row["equipo_a"],
                "codigo": str(row["codigo_a"]).strip().upper(),
            }
        )
        ...
        participantes.append(
            {
                "match_id": row["match_id"],
                "lado": "B",
                "seleccion": row["equipo_b"],
                "codigo": str(row["codigo_b"]).strip().upper(),
            }
        )

    participantes_df = pd.DataFrame(participantes)

    codigos_participantes = participantes_df["codigo"].tolist()
    codigos_participantes_set = set(codigos_participantes)

    duplicados = (
        participantes_df[
            participantes_df["codigo"].duplicated(keep=False)
        ]
        .sort_values(["codigo", "match_id"])
        .copy()
    )

    equipos_no_clasificados_en_r32 = participantes_df[
        participantes_df["codigo"].isin(codigos_no_clasificados)
    ].copy()

    equipos_fuera_de_lista_clasificados = participantes_df[
        ~participantes_df["codigo"].isin(codigos_clasificados)
    ].copy()

    clasificados_faltantes = sorted(
        list(codigos_clasificados - codigos_participantes_set)
    )

    errores = []

    if len(participantes_df) != 32:
        errores.append(
            f"Ronda de 32 debe tener 32 participantes. Tiene {len(participantes_df)}."
        )

    if not duplicados.empty:
        errores.append(
            "Hay equipos duplicados en Ronda de 32:\n"
            + duplicados.to_string(index=False)
        )

    if not equipos_no_clasificados_en_r32.empty:
        errores.append(
            "Hay equipos NO clasificados jugando Ronda de 32:\n"
            + equipos_no_clasificados_en_r32.to_string(index=False)
        )

    if not equipos_fuera_de_lista_clasificados.empty:
        errores.append(
            "Hay equipos en Ronda de 32 que no están en la lista de clasificados:\n"
            + equipos_fuera_de_lista_clasificados.to_string(index=False)
        )

    if clasificados_faltantes:
        errores.append(
            "Hay clasificados que no aparecen en Ronda de 32:\n"
            + ", ".join(clasificados_faltantes)
        )

    if errores:
        raise ValueError(
            "VALIDACIÓN FINAL FALLIDA - Ronda de 32 contra tabla de grupos\n\n"
            + "\n\n".join(errores)
        )

    print("====================================================")
    print("VALIDACIÓN FINAL RONDA DE 32")
    print("Estado: OK")
    print("Partidos Ronda de 32:", len(r32))
    print("Participantes únicos:", len(codigos_participantes_set))
    print("Equipos no clasificados en R32: 0")
    print("Equipos duplicados en R32: 0")
    print("Clasificados faltantes en R32: 0")
    print("====================================================")
...
def main():
    ...
    validar_ronda_32_contra_tabla_grupos(
