...
export default async function HousePage({ params }: { params: Promise<{ block: string; house: string }> }) {
  ...
  if (!Number.isInteger(block) || !Number.isInteger(house) || block <= 0 || house <= 0) notFound();

  const store = await getPaymentStore();
  const history = await getHouseHistory(store, block, house);
    ...
    <main className="shell">
      <header className="admin-head">
        <div>
          ...
          <h1 style={{ fontSize: 'clamp(2rem,5vw,3.6rem)' }}>Bloque {block} · Casa {house}</h1>
      ...
      <section className="kpis">
        ...
        <div className="kpi"><span>Períodos verificados</span><strong>{verifiedPeriods.size}</strong><small>Confirmados por fuente confiable</small></div>
        ...
        <div className="kpi"><span>Estado vivienda</span><strong>{history.home.active ? 'Activa' : 'Inactiva'}</strong><small>ID operativo B{block}-C{house}</small></div>
      </section>

      <div className="section-head">
        <div><p className="eyebrow">Historial</p><h2>Pagos y comprobantes</h2></div>
        <p>Los estados distinguen recepción de comprobante y verificación bancaria.</p>
      </div>
      <div className="table-wrap">
        <table>
          <thead><tr><th>Período</th><th>Fecha</th><th>Depositante</th><th>Monto</th><th>Referencia</th><th>Banco</th><th>Estado</th><th>Comprobante</th></tr></thead>
          <tbody>
            {history.payments.length === 0 && <tr><td colSpan={8}>Esta vivienda todavía no tiene pagos registrados.</td></tr>}
