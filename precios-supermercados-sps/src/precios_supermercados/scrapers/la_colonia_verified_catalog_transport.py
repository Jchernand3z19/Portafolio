"""Colección de páginas de catálogo ligada al plan edge autenticado.

Esta capa cierra el tramo entre ``AuthenticatedCatalogProvenancePlan`` y las
páginas que pueden alimentar la evidencia de cobertura. El caller no aporta
URLs, particiones, rangos ni ordenamientos: cada request se reconstruye desde el
árbol estructural verificado y la siguiente página exacta del plan autenticado.

La respuesta del gateway sólo se expone después de validar el body GraphQL y
verificar el receipt Ed25519. La colección sigue siendo una composición offline:
no implementa transporte HTTP productivo, no consulta Observability y nunca
concede ``production_authority`` ni aceptación comercial.
"""
...
class VerifiedCatalogPageObservation:
    """Página criptográficamente íntegra y evidencia cruda para cobertura."""
...
class VerifiedCatalogCollection:
    """Dos traversals completas derivadas de un único plan autenticado."""
...
class VerifiedCatalogEdgeCollector:
    """Ejecuta exclusivamente la siguiente página del plan autenticado exacto."""
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(authenticated_plan, AuthenticatedCatalogProvenancePlan):
            _fail("catalog_authenticated_plan_invalid")
        if authenticated_plan.production_authority is not False:
            _fail("catalog_authenticated_plan_authority_invalid")
        ...
        leaves = authenticated_plan.discovery.structure.valid_leaves
        leaf_by_name = {leaf.name: leaf for leaf in leaves}
        if len(leaf_by_name) != len(leaves):
            _fail("catalog_partition_names_not_unique")
        expected_partitions = {page.partition_id for page in authenticated_plan.pages}
    ...
    def _assert_unique_physical_identity(
        self,
        page: CryptographicallyVerifiedEdgeCatalogPage,
    ) -> None:
    ...
    def collect_next(self) -> VerifiedCatalogPageObservation:
        ...
        self._assert_unique_physical_identity(verified)
