...
function payment(period: string, id = `pay-${period}`): PaymentRecord {
  return {
    ...
    period, status: 'VERIFICADO', fileHash: `hash-${id}`,
  };
}
...
describe('service month assignment', () => {
  ...
  it('assigns a September deposit to September once August is paid', () => {
    expect(assignServicePeriod(home, '2026-09-01', [payment('2026-08')])).toBe('2026-09');
  });

  it('walks forward from August to the oldest unpaid month', () => {
    expect(assignServicePeriod(home, '2026-11-20', [payment('2026-08'), payment('2026-09')])).toBe('2026-10');
  });

  it('does not silently advance beyond the deposit month when all months are occupied', () => {
