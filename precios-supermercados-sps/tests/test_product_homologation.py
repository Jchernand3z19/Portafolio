...
def test_specific_taxonomy_rule_wins_over_generic_word() -> None:
    dental = assign_taxonomy(product("a", "walmart", "Pasta Dental Colgate Triple Acción 75 ml"))
...
def test_brand_difference_blocks_candidate_even_when_name_and_size_are_similar() -> None:
    result = homologate_products(
        (
            product("a", "colonial", "Arroz Progreso Blanco 1 lb", brand="Progreso", presentation="1 lb"),
            product("b", "paiz", "Arroz Great Value Blanco 454 g", brand="Great Value", presentation="454 g"),
        ),
...
def test_same_supermarket_never_creates_cross_source_candidate() -> None:
    result = homologate_products(
        (
            product("a", "colonial", "Café Passion Molido 1 lb", brand="Passion", presentation="1 lb"),
            product("b", "colonial", "Cafe Passion Molido 454 g", brand="Passion", presentation="454 g"),
        ),
        candidate_threshold=Decimal("0"),
    )
    assert result.candidates == ()
...
def test_summary_reports_classification_gtin_candidates_and_conflicts() -> None:
    result = homologate_products(
        (
            product("a", "colonial", "Huevos Rica Yema 15 und", brand="Rica Yema", presentation="15 und"),
            product("b", "walmart", "Huevos Rica Yema 15 unidades", brand="Rica Yema", presentation="15 unidades"),
            product(
                "c",
                "paiz",
