...
function bearerToken(request) {
  ...
  if (!token || token.trim() !== token || /\s/u.test(token) || token.length > 20_000) {
    fail("bearer_invalid");
  }
  return token;
}
...
function parseStructuralBody(body) {
  exactObject(body, ["originUrl", "requestContext"], "structural_execute_body_shape_invalid");
  ...
  const result = await executeStructuralGatewayRequest(
    ...
    },
    {
      ...
      verifyReceipt: async (payload, signature, keyId) => {
        ...
        return verifyStructuralReceipt(payload, signature, publicKey);
      },
      clock,
...
export function createStructuralPublicWorkerHandler({ namespace, authenticate }) {
  ...
  return async function handle(request) {
    try {
      ...
      if (!stub || typeof stub.structuralExecute !== "function") {
        fail("durable_structural_execute_rpc_missing");
      }
      const result = await stub.structuralExecute({
