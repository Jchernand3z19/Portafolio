...
const homes: HomeRecord[] = [
  { id: 'home-4-18', block: 4, house: 18, monthlyFee: 150, active: true },
  { id: 'home-4-19', block: 4, house: 19, monthlyFee: 150, active: true },
];

function payment(id: string, period: string, status: PaymentRecord['status'], house = 18): PaymentRecord {
  return {
    id,
    createdAt: `${period}-08T12:00:00.000Z`,
    updatedAt: `${period}-08T12:00:00.000Z`,
...
describe('house history', () => {
  ...
  it('shows verified, received, review, and pending states per home', async () => {
    const store = new MemoryPaymentStore({
      homes,
      payments: [
        payment('sep', '2026-09', 'VERIFICADO'),
        payment('aug', '2026-08', 'PENDIENTE_VERIFICACION'),
        payment('jul', '2026-07', 'EN_REVISION'),
      ],
    });
  ...
  it('returns a house payment history newest period first without hiding trace records', async () => {
    const store = new MemoryPaymentStore({
      homes,
      payments: [
        payment('old', '2026-08', 'VERIFICADO'),
        payment('new', '2026-09', 'PENDIENTE_VERIFICACION'),
        payment('dup', '2026-09', 'DUPLICADO'),
      ],
    });
    const history = await getHouseHistory(store, 4, 18);
    expect(history.home?.id).toBe('home-4-18');
